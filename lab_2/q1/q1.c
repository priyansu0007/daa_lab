...
void insert_UA(int* arr, int* size, int x) { 
    arr[(*size)++] = x; 
}
...
int pred_UA(int* arr, int size, int index) {
    ...
    for (int i = 0; i < size; i++) {
        if (arr[i] < target && arr[i] > pred_val) { 
            pred_val = arr[i]; 
            pred_idx = i; 
        }
    }
...
int succ_UA(int* arr, int size, int index) {
    ...
    for (int i = 0; i < size; i++) {
        if (arr[i] > target && arr[i] < succ_val) { 
            succ_val = arr[i]; 
            succ_idx = i; 
        }
    }
...
SLLNode* pred_SLLU(SLLNode* head, SLLNode* target) {
    SLLNode *predNode = NULL; 
    int max_val = INT_MIN;
    
    while (head) {
        if (head->key < target->key && head->key > max_val) { 
            max_val = head->key; 
            predNode = head; 
        }
        head = head->next;
...
SLLNode* succ_SLLU(SLLNode* head, SLLNode* target) {
    SLLNode *succNode = NULL; 
    int min_val = INT_MAX;
    
    while (head) {
        if (head->key > target->key && head->key < min_val) { 
            min_val = head->key; 
            succNode = head; 
        }
        head = head->next;
...
void delete_SLLS(SLLNode** head, SLLNode* target) { 
    delete_SLLU(head, target); 
}
...
SLLNode* succ_SLLS(SLLNode* target) { 
...
// Macro to simplify time measurement
#define MEASURE_TIME(out_time, statement) \
    do { \
        clock_t _start = clock(); \
        statement; \
        clock_t _end = clock(); \
        out_time = time_diff(_start, _end); \
    } while(0)
...
int main() {
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        // 1. UNSORTED ARRAY
        // ------------------------------------------
        int* ua = (int*)malloc(N * sizeof(int)); 
        int ua_s = 0;
        
        MEASURE_TIME(t_ins,  for(int i=0; i<N; i++) insert_UA(ua, &ua_s, keys[i]));
        MEASURE_TIME(t_srch, for(int i=0; i<N; i++) search_UA(ua, ua_s, -1)); // Worst case
        MEASURE_TIME(t_max,  for(int i=0; i<N; i++) max_UA(ua, ua_s));
        MEASURE_TIME(t_min,  for(int i=0; i<N; i++) min_UA(ua, ua_s));
        MEASURE_TIME(t_prd,  for(int i=0; i<N; i++) pred_UA(ua, ua_s, i));
        MEASURE_TIME(t_suc,  for(int i=0; i<N; i++) succ_UA(ua, ua_s, i));
        MEASURE_TIME(t_del,  for(int i=0; i<N; i++) delete_UA(ua, &ua_s, 0)); // Delete idx 0
        ...
        MEASURE_TIME(t_ins,  for(int i=0; i<N; i++) insert_SA(sa, &sa_s, keys[i]));
        MEASURE_TIME(t_srch, for(int i=0; i<N; i++) search_SA(sa, sa_s, -1));
        MEASURE_TIME(t_max,  for(int i=0; i<N; i++) max_SA(sa, sa_s));
        MEASURE_TIME(t_min,  for(int i=0; i<N; i++) min_SA(sa, sa_s));
        MEASURE_TIME(t_prd,  for(int i=0; i<N; i++) pred_SA(sa_s, i));
        MEASURE_TIME(t_suc,  for(int i=0; i<N; i++) succ_SA(sa_s, i));
        MEASURE_TIME(t_del,  for(int i=0; i<N; i++) delete_SA(sa, &sa_s, 0)); // Worst case O(N) shift
        ...
        // ------------------------------------------
        SLLNode* sllu = NULL; 
        SLLNode* sllu_nodes[10000];
        
        MEASURE_TIME(t_ins, for(int i=0; i<N; i++) { 
            insert_SLLU(&sllu, keys[i]); 
            sllu_nodes[i] = sllu; 
        });
        MEASURE_TIME(t_srch, for(int i=0; i<N; i++) search_SLLU(sllu, -1));
        MEASURE_TIME(t_max,  for(int i=0; i<N; i++) max_SLLU(sllu));
        MEASURE_TIME(t_min,  for(int i=0; i<N; i++) min_SLLU(sllu));
        MEASURE_TIME(t_prd,  for(int i=0; i<N; i++) pred_SLLU(sllu, sllu_nodes[i]));
        MEASURE_TIME(t_suc,  for(int i=0; i<N; i++) succ_SLLU(sllu, sllu_nodes[i]));
        MEASURE_TIME(t_del,  for(int i=0; i<N; i++) delete_SLLU(&sllu, sllu_nodes[i]));
        ...
        // ------------------------------------------
        SLLNode* slls = NULL; 
        SLLNode* slls_nodes[10000];
        
        MEASURE_TIME(t_ins, for(int i=0; i<N; i++) insert_SLLS(&slls, keys[i]));
        ...
        MEASURE_TIME(t_srch, for(int i=0; i<N; i++) search_SLLS(slls, -1));
        MEASURE_TIME(t_max,  for(int i=0; i<N; i++) max_SLLS(slls));
        MEASURE_TIME(t_min,  for(int i=0; i<N; i++) min_SLLS(slls));
        MEASURE_TIME(t_prd,  for(int i=1; i<N; i++) pred_SLLS(slls, slls_nodes[i]));
        MEASURE_TIME(t_suc,  for(int i=0; i<N; i++) succ_SLLS(slls_nodes[i]));
        MEASURE_TIME(t_del,  for(int i=0; i<N; i++) delete_SLLS(&slls, slls_nodes[N-1-i])); // tail worst case
        ...
        // ------------------------------------------
        DLLNode* dllu = NULL; 
        DLLNode* dllu_nodes[10000];
        
        MEASURE_TIME(t_ins, for(int i=0; i<N; i++) { 
            insert_DLLU(&dllu, keys[i]); 
            dllu_nodes[i] = dllu; 
        });
        MEASURE_TIME(t_srch, for(int i=0; i<N; i++) search_DLLU(dllu, -1));
        MEASURE_TIME(t_max,  for(int i=0; i<N; i++) max_DLLU(dllu));
        MEASURE_TIME(t_min,  for(int i=0; i<N; i++) min_DLLU(dllu));
        MEASURE_TIME(t_prd,  for(int i=0; i<N; i++) pred_DLLU(dllu, dllu_nodes[i]));
        MEASURE_TIME(t_suc,  for(int i=0; i<N; i++) succ_DLLU(dllu, dllu_nodes[i]));
        MEASURE_TIME(t_del,  for(int i=0; i<N; i++) delete_DLLU(&dllu, dllu_nodes[i])); // O(1) delete
        ...
        DLLNode* dlls = NULL; 
        DLLNode* tail = NULL; 
        DLLNode* dlls_nodes[10000];
        
        MEASURE_TIME(t_ins, for(int i=0; i<N; i++) insert_DLLS(&dlls, &tail, keys[i]));
        ...
        MEASURE_TIME(t_srch, for(int i=0; i<N; i++) search_DLLS(dlls, -1));
        MEASURE_TIME(t_max,  for(int i=0; i<N; i++) max_DLLS(tail));
        MEASURE_TIME(t_min,  for(int i=0; i<N; i++) min_DLLS(dlls));
        MEASURE_TIME(t_prd,  for(int i=0; i<N; i++) pred_DLLS(dlls_nodes[i])); // O(1) using prev
        MEASURE_TIME(t_suc,  for(int i=0; i<N; i++) succ_DLLS(dlls_nodes[i])); // O(1) using next
        MEASURE_TIME(t_del,  for(int i=0; i<N; i++) delete_DLLS(&dlls, &tail, dlls_nodes[i])); // O(1) delete
